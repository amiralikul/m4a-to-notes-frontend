...
import { CancelSubscription, CancellationSuccess } from '@/components/cancel-subscription';
...
export default function SubscriptionPage() {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'trialing': return <Clock className="w-5 h-5 text-yellow-500" />;
  ...
  const getPlanIcon = (plan) => {
    switch (plan) {
      case 'pro': return <Crown className="w-6 h-6 text-blue-500" />;
      case 'business': return <Sparkles className="w-6 h-6 text-purple-500" />;
  ...
  const isSubscriptionActive = ['active', 'trialing'].includes(entitlements?.status);
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <div className="text-3xl font-bold">
                    ${currentPlan.price}
                    {currentPlan.price > 0 && <span className="text-sm text-gray-500 font-normal">/month</span>}
                  </div>
                </div>
              </div>
            </CardHeader>
                  ...
                  <div className="grid md:grid-cols-2 gap-4 text-sm">
                    ...
                    {entitlements.meta.periodEnd && (
                      <div>
                        <span className="font-medium text-gray-500">
                          {isSubscriptionActive ? 'Next billing date' : 'Expires on'}
                        </span>
                        <div className="flex items-center gap-1 mt-1">
                          <Calendar className="w-4 h-4 text-gray-400" />
                          <span>{new Date(entitlements.meta.periodEnd).toLocaleDateString()}</span>
                        </div>
                      </div>
                    )}

                    {entitlements.meta.currency && entitlements.meta.unitPrice && (
                      <div>
                        <span className="font-medium text-gray-500">Amount</span>
                        <div className="flex items-center gap-1 mt-1">
                          <CreditCard className="w-4 h-4 text-gray-400" />
                          <span>
                            {entitlements.meta.currency} {(entitlements.meta.unitPrice / 100).toFixed(2)}
                    ...
                    <div>
                      ...
                      <div className="text-gray-600 mt-1">
                        {new Date(entitlements.updatedAt).toLocaleDateString()}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Track your transcription usage and limits</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  <div key={plan.key} className="border rounded-lg p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-blue-600">${plan.price}/mo</p>
                      </div>
                    </div>

...
export default function FileUpload() {
	...
	const processFileWithAPI = useCallback(async (fileId, file) => {
		try {
			...
			// Update with transcription ID and start polling
			setUploadedFiles((prev) =>
				prev.map((f) =>
					f.id === fileId
						? {
	...
	const pollTranscriptionStatus = async (fileId, transcriptionId) => {
		...
		const poll = async () => {
			...
			if (pollCount >= maxPollCount) {
				console.warn(
					`Polling timeout for transcription ${transcriptionId} after ${MAX_POLLING_TIME / 1000} seconds`,
				);
				setUploadedFiles((prev) =>
					prev.map((f) =>
						f.id === fileId
			...
			try {
				...
				if (!response.ok) {
					throw new Error(
						transcriptionStatus.error || "Failed to get transcription status",
					);
				}
				...
				setUploadedFiles((prev) =>
					prev.map((f) =>
						f.id === fileId
							? {
									...f,
									progress: transcriptionStatus.progress || 50,
								}
							: f,
					),
				);
				...
				if (transcriptionStatus.status === "completed") {
					// Get transcript text directly from status response (it includes preview)
					// For full transcript, we can use the preview or fetch from transcript endpoint
					let transcriptText = transcriptionStatus.preview || "";

					// If we want the full transcript, fetch from transcript endpoint
					try {
						...
						console.warn(
							"Could not fetch full transcript, using preview:",
							error,
						);
						transcriptText =
										...
										<div className="flex-1 min-w-0">
											...
											<div className="flex items-center space-x-4 text-sm text-gray-500 mt-1">
												...
												{uploadedFile.duration && (
													<span>Duration: {uploadedFile.duration}</span>
												)}
											</div>
										</div>
									</div>
										...
										<div className="flex items-center justify-between mb-3">
											...
											<Button
												...
												onClick={() => {
													...
													const file = new Blob([uploadedFile.transcription], {
														type: "text/plain",
													});
